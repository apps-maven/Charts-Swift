...
var arrMonths = ["Jan","Feb","Mar","April","May","June","July","Aug","Sept","Oct","Nov","Dec"]
...
open class Chart: UIView {
    ...
    */
    ...
    fileprivate func drawChart() {
        ...
        for (index, series) in self.series.enumerated() {
            ...
            segments.forEach({ segment in
                ...
                if series.line {
                    drawLine(scaledXValues, yValues: scaledYValues, seriesIndex: index, colorForDot: nil)
                }
                if series.area {
    ...
    fileprivate func drawLine(_ xValues: [Double], yValues: [Double], seriesIndex: Int,colorForDot: UIColor?) {
        ...
        let layer = UIView.init(frame: CGRect.init(x: xValues.first!+0.5, y: yValues.first!-2.5 , width: 8, height: 8))
        
        layer.layer.cornerRadius = 4
        
        layer.layer.masksToBounds = true
        
        layer.backgroundColor = series[seriesIndex].boolColor
        
        print(self.addSubview(layer))
...
//           print(yValues[i])
            ...
            let layer = UIView.init(frame: CGRect.init(x: xValues[i]-2.5, y: yValues[i]-2.5 , width: 8, height: 8))
            ...
            let y = yValues[i]
//            path.addLine(to: CGPoint(x: CGFloat(xValues[i]), y: CGFloat(y))) //for line
        }
...
//        lineLayer.lineJoin = CAShapeLayerLineJoin.round
    ...
    fileprivate func drawLabelsAndGridOnXAxis() {
        ...
        scaled.enumerated().forEach { (i, value) in
            ...
            label.text = arrMonths[i]
            
            //"dfghjr"//xLabelsFormatter(i, labels[i])
...
//            if y != drawingHeight + topInset && y != zero {
                ...
                context.move(to: CGPoint(x: CGFloat(0), y: y))
                context.addLine(to: CGPoint(x: self.bounds.width+100, y: y))
//                if labels[i] != 0 {
//                    // Horizontal grid for 0 is not dashed
//                    context.setLineDash(phase: CGFloat(0), lengths: [CGFloat(5)])
//                } else {
//                    context.setLineDash(phase: CGFloat(0), lengths: [CGFloat(50)])
//                }
                context.strokePath()
//            }

            let label = UILabel(frame: CGRect(x: padding, y: y+5, width: 0, height: 0))
